...
public class DoublyLinkedList<E extends PubliclyCloneable> implements PubliclyCloneable {
  ...
  public E removeTarget(E target){
	  if(getTargetNode(target)==null)
		  return null;
	  return remove(getTargetNode(target));
  }
  ...
  public DoublyLinkedList<E> clone2() throws CloneNotSupportedException {
	    ...
	    if (size > 0) {
	        ...
	        for(int i = 0; i < size; i++) {
	            Node<E> newest = new Node<>((E)(walk.getElement()).clone(), otherWalk, otherWalk.getNext());
